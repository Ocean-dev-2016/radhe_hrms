<?php
$pageTitle = "Import Shift Timing - Payroll System";
include 'header.php';
?>
  ...
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-clock-play me-2" style="font-size: 16px;"></i>IMPORT SHIFT & TIMING FROM EXCEL SHEET
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="importTabs" role="tablist"
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="import-tab" type="button" style="font-size: 11px;">Shift
            Timing Import</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <a href="actions/import-shift-action.php?action=download_current" target="_blank"
            ...
            <i class="ti ti-download me-1"></i>Download Current Shifts
          </a>
          <a href="actions/import-shift-action.php?action=format_file" target="_blank"
            class="btn btn-sm btn-outline-secondary px-3"
            style="font-size: 11px; height: 28px; border-color: #a3b8cc !important; background-color: #ffffff; color: #135ca3; font-weight: 600;">
          ...
          <table class="table table-bordered table-sm m-0 text-nowrap table-hover align-middle" id="previewTable"
            ...
            <thead class="sticky-top bg-light" style="z-index: 1;">
              <tr id="tableHeaderRow" style="background-color: #f1f5f9;">
                ...
                <th class="px-2 py-1 text-center">Shift Code</th>
                <th class="px-2 py-1">Shift Name</th>
                <th class="px-2 py-1 text-center">Start Time</th>
                <th class="px-2 py-1 text-center">End Time</th>
                <th class="px-2 py-1 text-center">Effective Date</th>
            ...
            <tbody id="tableBody">
              <tr>
                <td colspan="10" class="text-center py-5 text-muted">
                  ...
                  Please browse and load an Excel/CSV file to preview shift data.
                </td>
              </tr>
            </tbody>
          </table>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btnLoadExcel.addEventListener('click', () => {
      ...
      fetch('actions/import-shift-action.php?action=load_excel', {
        method: 'POST',
        body: formData
      })
    ...
    function renderPreviewTable(rows) {
      ...
      rows.forEach((row, idx) => {
        ...
        html += `
                <tr class="${rowClass}" data-filter-text="${(row.emp_code + ' ' + row.emp_name + ' ' + row.shift_code + ' ' + row.shift_name).toLowerCase()}">
                    <td class="text-center">${idx + 1}</td>
                    <td><strong>${escapeHtml(row.emp_code || '')}</strong></td>
                    <td>${escapeHtml(row.emp_name || '')}</td>
                    <td class="text-center"><span class="badge bg-label-primary" style="font-size: 10px;">${escapeHtml(row.shift_code || '')}</span></td>
                    <td>${escapeHtml(row.shift_name || '')}</td>
                    <td class="text-center fw-semibold text-success">${escapeHtml(row.start_time || '')}</td>
                    <td class="text-center fw-semibold text-danger">${escapeHtml(row.end_time || '')}</td>
                    <td class="text-center">${escapeHtml(row.effective_date || '')}</td>
    ...
    btnUploadData.addEventListener('click', () => {
      ...
      if (!confirm(`Are you sure you want to upload shift assignments for ${validRowsToUpload.length} employees?`)) return;
      ...
      fetch('actions/import-shift-action.php?action=upload_data', {
        method: 'POST',
        body: formData
      })
        .then(response => response.json())

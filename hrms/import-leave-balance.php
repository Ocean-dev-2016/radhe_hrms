<?php
$pageTitle = "Leave Balance Upload - Payroll System";
include 'header.php';
  ...
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-file-text me-2" style="font-size: 16px;"></i>LEAVE BALANCE UPLOAD FROM EXCEL SHEET
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="importTabs" role="tablist"
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="import-tab" type="button"
            style="font-size: 11px;">Leave Balance Upload</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          ...
          <a href="actions/import-leave-balance-action.php?action=download_current&year=<?php echo $current_year; ?>" id="btnDownloadCurrent" target="_blank"
          ...
          <a href="actions/import-leave-balance-action.php?action=format_file" target="_blank"
            class="btn btn-sm btn-outline-secondary px-3"
            style="font-size: 11px; height: 28px; border-color: #a3b8cc !important; background-color: #ffffff; color: #135ca3; font-weight: 600;">
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    yearSelect.addEventListener('change', () => {
      btnDownloadCurrent.href = `actions/import-leave-balance-action.php?action=download_current&year=${yearSelect.value}`;
    });
    ...
    btnLoadExcel.addEventListener('click', () => {
      ...
      fetch('actions/import-leave-balance-action.php?action=load_excel', {
        method: 'POST',
        body: formData
      })
    ...
    btnUploadData.addEventListener('click', () => {
      ...
      if (!confirm(`Are you sure you want to upload leave balance for ${validRowsToUpload.length} employees?`)) {
        return;
      }
      ...
      fetch('actions/import-leave-balance-action.php?action=upload_data', {
        method: 'POST',
        body: postData
      })
        .then(response => response.json())

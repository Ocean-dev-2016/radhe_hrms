<?php
$pageTitle = "Salary Delete Log - Payroll System";
require_once 'root/config.php';
include 'header.php';
global $ai_db;

$company_id = isset($_SESSION['selected_company_id']) ? intval($_SESSION['selected_company_id']) : 0;
$logs = $ai_db->aiGetQuery("SELECT * FROM hrms_salary_delete_log WHERE company_id = $company_id ORDER BY deleted_at DESC");
  ...
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-receipt-off me-2" style="font-size: 16px;"></i>SALARY DELETION AUDIT LOG
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="logTabs" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="log-tab" type="button" style="font-size: 11px;">Deleted Salary Records</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
          <div>
            <span class="badge bg-primary" style="font-size: 11px;">Total Salary Deletion Records: <?php echo count($logs); ?></span>
          </div>
          ...
          <table class="table table-bordered table-sm m-0 text-nowrap table-hover align-middle" id="logTable"
            ...
            <thead class="sticky-top bg-light" style="z-index: 1;">
              <tr style="background-color: #f1f5f9;">
                ...
                <th class="px-2 py-1 text-center">Period</th>
              ...
              <?php if (empty($logs)): ?>
                <tr>
                  <td colspan="9" class="text-center py-5 text-muted">
                    ...
                    No salary deletion records found in log history.
                  </td>
                </tr>
              <?php else: ?>
                <?php foreach ($logs as $idx => $l): ?>
                  <tr data-filter-text="<?php echo strtolower($l['emp_code'] . ' ' . $l['emp_name'] . ' ' . $l['deleted_by'] . ' ' . $l['deleted_reason']); ?>">
                    ...
                    <td class="text-center font-monospace"><?php echo ($month_names[$l['salary_month']] ?? $l['salary_month']) . '-' . $l['salary_year']; ?></td>
                    <td class="text-end fw-semibold text-danger">Rs. <?php echo number_format($l['gross_salary'], 2); ?></td>
                    <td class="text-end fw-bold text-danger">Rs. <?php echo number_format($l['net_salary'], 2); ?></td>

<?php
$pageTitle = "Import Nominee - Payroll System";
include 'header.php';
?>
  ...
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-users me-2" style="font-size: 16px;"></i>IMPORT EMPLOYEE NOMINEE FROM EXCEL SHEET
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="importTabs" role="tablist"
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="import-tab" type="button"
            style="font-size: 11px;">Nominee Import</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <a href="actions/import-nominee-action.php?action=download_current" target="_blank"
          ...
          <a href="actions/import-nominee-action.php?action=format_file" target="_blank"
            class="btn btn-sm btn-outline-secondary px-3"
            style="font-size: 11px; height: 28px; border-color: #a3b8cc !important; background-color: #ffffff; color: #135ca3; font-weight: 600;">
          ...
          <div class="d-flex align-items-center ms-auto" style="max-width: 620px; width: 100%;">
            <input type="text" id="fileNameDisplay" class="form-control form-control-sm bg-white border me-2" readonly
              placeholder="No file chosen (.xlsx, .xls, .csv)" style="font-size: 11px; height: 28px; border: 1px solid #135ca3 !important;">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 px-1">
          ...
          <div class="d-flex align-items-center" style="max-width: 250px;">
            <input type="text" id="tableFilterInput" class="form-control form-control-sm bg-white border"
              placeholder="Search in preview..." style="font-size: 11px; height: 26px; border-color: #a3b8cc !important;">
          </div>
        </div>
          ...
          <table class="table table-bordered table-sm m-0 text-nowrap table-hover align-middle" id="previewTable"
            ...
            <thead class="sticky-top bg-light" style="z-index: 1;">
              <tr id="tableHeaderRow" style="background-color: #f1f5f9;">
                ...
                <th class="px-2 py-1">Nominee Name</th>
                <th class="px-2 py-1">Relation</th>
                <th class="px-2 py-1 text-center">Birth Date</th>
                <th class="px-2 py-1 text-end">Share (%)</th>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btnLoadExcel.addEventListener('click', () => {
      ...
      fetch('actions/import-nominee-action.php?action=load_excel', {
        method: 'POST',
        body: formData
      })
    ...
    function renderPreviewTable(rows) {
      ...
      rows.forEach((row, idx) => {
        ...
        html += `
                <tr class="${rowClass}" data-filter-text="${(row.emp_code + ' ' + row.emp_name + ' ' + row.nominee_name + ' ' + row.relation).toLowerCase()}">
                    <td class="text-center">${idx + 1}</td>
                    <td><strong>${escapeHtml(row.emp_code || '')}</strong></td>
                    <td>${escapeHtml(row.emp_name || '')}</td>
                    <td><strong>${escapeHtml(row.nominee_name || '')}</strong></td>
                    <td>${escapeHtml(row.relation || '')}</td>
                    <td class="text-center">${escapeHtml(row.birth_date || '')}</td>
                    <td class="text-end fw-semibold">${parseFloat(row.share_percentage || 0).toFixed(2)}%</td>
    ...
    btnUploadData.addEventListener('click', () => {
      ...
      if (!confirm(`Are you sure you want to upload ${validRowsToUpload.length} Nominee records?`)) return;
      ...
      fetch('actions/import-nominee-action.php?action=upload_data', {
        method: 'POST',
        body: formData
      })
        .then(response => response.json())

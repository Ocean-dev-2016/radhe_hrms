<?php
$pageTitle = "Import Increment - Payroll System";
include 'header.php';
?>
  ...
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-trending-up me-2" style="font-size: 16px;"></i>IMPORT SALARY INCREMENT FROM EXCEL SHEET
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="importTabs" role="tablist"
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="import-tab" type="button"
            style="font-size: 11px;">Salary Increment Import</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <a href="actions/import-increment-action.php?action=download_current" target="_blank"
          ...
          <a href="actions/import-increment-action.php?action=format_file" target="_blank"
            class="btn btn-sm btn-outline-secondary px-3"
            style="font-size: 11px; height: 28px; border-color: #a3b8cc !important; background-color: #ffffff; color: #135ca3; font-weight: 600;">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-2 px-1">
          <div class="d-flex align-items-center gap-2">
            ...
            <span class="badge bg-info" id="badgeTotalInc" style="font-size: 11px;">Total Inc. Basic: ₹0.00</span>
          </div>
          ...
          <table class="table table-bordered table-sm m-0 text-nowrap table-hover align-middle" id="previewTable"
            ...
            <thead class="sticky-top bg-light" style="z-index: 1;">
              <tr id="tableHeaderRow" style="background-color: #f1f5f9;">
                <th class="px-2 py-1 text-center" style="width: 40px;">Sr</th>
                <th class="px-2 py-1">Code</th>
                <th class="px-2 py-1">Name</th>
                <th class="px-2 py-1 text-center">Type</th>
                <th class="px-2 py-1 text-end text-primary fw-bold">Basic (₹)</th>
                <th class="px-2 py-1 text-end">HRA (₹)</th>
                <th class="px-2 py-1 text-end">Medical (₹)</th>
                <th class="px-2 py-1 text-end">Conve (₹)</th>
                <th class="px-2 py-1 text-end">Education (₹)</th>
                <th class="px-2 py-1 text-end">Washing (₹)</th>
                <th class="px-2 py-1 text-end">Paper (₹)</th>
                <th class="px-2 py-1 text-end">Recovery (₹)</th>
                <th class="px-2 py-1 text-end">City (₹)</th>
                <th class="px-2 py-1 text-end">Atten (₹)</th>
                <th class="px-2 py-1 text-end">Other Earn (₹)</th>
                <th class="px-2 py-1 text-end">Other Ded (₹)</th>
                <th class="px-2 py-1 text-end">Leave Allow (₹)</th>
                <th class="px-2 py-1 text-end">Bonus (₹)</th>
                <th class="px-2 py-1 text-end">Gratuity (₹)</th>
                <th class="px-2 py-1 text-center">Year/Month</th>
            ...
            <tbody id="tableBody">
              <tr>
                <td colspan="21" class="text-center py-5 text-muted">
                  ...
                  Please browse and load an Excel/CSV file to preview increment data.
                </td>
              </tr>
            </tbody>
          </table>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const badgeTotalInc = document.getElementById('badgeTotalInc');
    ...
    btnLoadExcel.addEventListener('click', () => {
      ...
      fetch('actions/import-increment-action.php?action=load_excel', {
        method: 'POST',
        body: formData
      })
        ...
        .then(res => {
          if (res.status === 'success') {
            ...
            if (res.summary) {
              ...
              badgeTotalInc.textContent = `Total Basic Inc.: ₹${parseFloat(res.summary.total_inc_amt || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
            }
        ...
        .catch(err => {
          console.error(err);
          tableBody.innerHTML = `<tr><td colspan="21" class="text-center py-5 text-danger">Failed to communicate with server.</td></tr>`;
          btnUploadData.disabled = true;
        });
    });
    ...
    function renderPreviewTable(rows) {
      if (rows.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="21" class="text-center py-5 text-warning">No rows found in Excel sheet.</td></tr>`;
        return;
      }
      ...
      rows.forEach((row, idx) => {
        ...
        html += `
                <tr class="${rowClass}" data-filter-text="${(row.emp_code + ' ' + row.emp_name).toLowerCase()}">
                    <td class="text-center">${idx + 1}</td>
                    <td><strong>${escapeHtml(row.emp_code || '')}</strong></td>
                    <td>${escapeHtml(row.emp_name || '')}</td>
                    <td class="text-center"><span class="badge bg-label-info" style="font-size: 10px;">${escapeHtml(row.pay_type || 'MONTHLY')}</span></td>
                    <td class="text-end fw-bold text-primary">${parseFloat(row.basic || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.hra || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.medical || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.conve || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.education || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.washing || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.paper || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.recovery || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.city || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.atten || 0).toFixed(2)}</td>
                    <td class="text-end">${parseFloat(row.other_earn || 0).toFixed(2)}</td>
                    <td class="text-end text-danger">${parseFloat(row.other_ded || 0).toFixed(2)}</td>
    ...
    btnUploadData.addEventListener('click', () => {
      ...
      if (!confirm(`Are you sure you want to apply salary increments for ${validRowsToUpload.length} employees?`)) return;

      btnUploadData.disabled = true;
      btnUploadData.innerHTML = '<span class="spinner-border spinner-border-sm text-primary me-2" role="status"></span>Updating...';

      const formData = new FormData();
      formData.append('data', JSON.stringify(validRowsToUpload));

      fetch('actions/import-increment-action.php?action=upload_data', {
        method: 'POST',
        body: formData
      })
        .then(response => response.json())
        .then(res => {
          ...
          if (res.status === 'success') {
            ...
            tableBody.innerHTML = `
                  <tr>
                    <td colspan="12" class="text-center py-5 text-success">

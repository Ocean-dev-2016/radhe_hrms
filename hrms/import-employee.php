<?php
$pageTitle = "Employee Import - Payroll System";
include 'header.php';
?>
...
<div class="container-fluid flex-grow-1 container-p-y position-relative" style="min-height: calc(100vh - 120px);">
  ...
  <!-- Floating Dialog Card (Non-draggable for full screen import grid or styled classically) -->
  <div class="card shadow-lg border-1"
    ...
    <div class="card-header p-2 px-3 text-white d-flex align-items-center justify-content-between"
      ...
      <h6 class="m-0 text-white fw-bold d-flex align-items-center" style="font-size: 14px;">
        <i class="ti ti-file-import me-2" style="font-size: 16px;"></i>EMPLOYEE IMPORT OR UPDATE FROM EXCEL SHEET
      </h6>
    ...
    <div class="card-body p-3 bg-white">
      ...
      <ul class="nav nav-tabs mb-0 border-bottom-0" id="importTabs" role="tablist"
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link active fw-bold py-1 px-3" id="import-tab" type="button"
            style="font-size: 11px;">Employee Import or Update</button>
        </li>
      </ul>
      ...
      <div class="border p-3 rounded-bottom bg-legacy-blue">
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
          <a href="actions/import-employee-action.php?action=download_current" target="_blank"
          ...
          <a href="actions/import-employee-action.php?action=format_file" target="_blank"
            class="btn btn-sm btn-outline-secondary px-3"
            style="font-size: 11px; height: 28px; border-color: #a3b8cc !important; background-color: #ffffff; color: #135ca3; font-weight: 600;">
            Format File
          </a>
          ...
          <table class="table table-bordered table-sm m-0 text-nowrap table-hover" id="previewTable"
            ...
            <thead class="sticky-top bg-light" style="z-index: 1;">
              <tr id="tableHeaderRow" style="background-color: #f1f5f9;">
                ...
                <th class="px-2 py-1">Emp Name</th>
                ...
                <th class="px-2 py-1">Joining Date</th>
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    btnLoadExcel.addEventListener('click', () => {
      ...
      fetch('actions/import-employee-action.php?action=load_excel', {
        method: 'POST',
        body: formData
      })
    ...
    function renderPreviewTable(rows) {
      if (rows.length === 0) {
        tableBody.innerHTML = `
                <tr>
                    <td colspan="9" class="text-center py-5 text-warning">
      ...
      rows.forEach((row, idx) => {
        html += `
                <tr>
                    ...
                    <td>
                        <span class="badge ${row.status === 'active' ? 'bg-success' : 'bg-secondary'} px-2 py-1">
                            ${row.status || 'active'}
                        </span>
                    </td>
                </tr>
    ...
    btnUploadData.addEventListener('click', () => {
      ...
      fetch('actions/import-employee-action.php?action=upload_data', {
        method: 'POST',
        body: formData
      })
        .then(response => response.json())
